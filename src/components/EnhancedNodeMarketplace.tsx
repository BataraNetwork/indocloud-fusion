        ...
        <div className="relative p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-cyber-green font-medium">2,847 Verified Nodes</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <TrendingUp className="w-4 h-4 text-cyber-cyan" />
              <span className="text-cyber-cyan font-medium">99.7% Avg Uptime</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <Zap className="w-4 h-4 text-cyber-purple" />
              <span className="text-cyber-purple font-medium">1,523 Active Jobs</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <Globe className="w-4 h-4 text-cyber-pink" />
              <span className="text-cyber-pink font-medium">47 Countries</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <Select value={priceFilter} onValueChange={setPriceFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="low">< 1 INDO</SelectItem>
                <SelectItem value="medium">1-5 INDO</SelectItem>
                <SelectItem value="high">> 5 INDO</SelectItem>
              </SelectContent>
            </Select>
        ...
        <TabsContent value="storage" className="space-y-4 mt-6">
          <div className="grid gap-6">
            {storageNodes.map((node) => (
              <Card key={node.id} className="glass-card hover-lift glow-cyber">
                <CardContent className="p-6">
                  <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-xl font-semibold gradient-text">{node.name}</h3>
                            {node.verified && (
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-sm text-muted-foreground">({node.reviews} reviews)</span>
                            </div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-2xl font-bold text-cyber-purple">{node.price}</div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                        <div>
                          ...
                          <p className="font-medium text-cyber-cyan">{node.storage}</p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">Available</p>
                          <p className="font-medium text-cyber-green">{node.available}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-medium text-cyber-green">{node.uptime}%</p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">Reputation</p>
                          <p className="font-medium text-cyber-purple">{node.reputation.toLocaleString()}</p>
                        </div>
                      </div>

                      <div className="flex flex-wrap gap-2 mb-4">
                        {node.features.map((feature, index) => (
                          <Badge key={index} variant="outline" className="border-cyber-cyan/30 text-cyber-cyan">
                            {feature}
                          </Badge>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>Total Earned: <span className="text-cyber-green font-medium">{node.totalEarned}</span></span>
                        <span>Peak Load: <span className="text-cyber-orange font-medium">{node.peakLoad}</span></span>
                    ...
                    <div className="lg:text-right space-y-3">
                      ...
                      <Button variant="ghost" size="sm" className="w-full text-cyber-cyan hover:bg-cyber-cyan/10">
        ...
        <TabsContent value="compute" className="space-y-4 mt-6">
          <div className="grid gap-6">
            {computeNodes.map((node) => (
              <Card key={node.id} className="glass-card hover-lift glow-neon">
                <CardContent className="p-6">
                  <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                    {/* Enhanced Node Info */}
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="text-right">
                          <div className="text-2xl font-bold text-cyber-cyan">{node.price}</div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                          <p className="text-xs text-muted-foreground">Hardware Specs</p>
                          <p className="font-medium text-cyber-cyan">{node.specs}</p>
                        </div>
                    ...
                    <div className="lg:text-right space-y-3">
                      ...
                      <Button variant="ghost" size="sm" className="w-full text-cyber-purple hover:bg-cyber-purple/10">
                        <Activity className="w-4 h-4 mr-2" />
                        Live Metrics
                      </Button>
                    </div>
